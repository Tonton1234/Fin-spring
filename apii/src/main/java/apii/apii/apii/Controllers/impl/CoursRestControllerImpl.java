...
public class CoursRestControllerImpl implements CoursRestController {
    ...
    @Override
    public ResponseEntity<Map<Object, Object>> listerCoursByProf(String data, int page, int size, String keyword) {
        Page<Cours> cours=null;

        if (Objects.equals(keyword, "Planifier")){
            System.out.println("ok");
            cours= cours=coursRepository.findCoursByEtatAndProfesseurAndActiveTrue(EtatCours.Encours,professeurRepository.findByUsername(data),PageRequest.of(page,size));
        }
        if (Objects.equals(keyword, "Terminer")){
            System.out.println("fin");
            cours= cours=coursRepository.findCoursByEtatAndProfesseurAndActiveTrue(EtatCours.Terminer,professeurRepository.findByUsername(data),PageRequest.of(page,size));
        }
        if (Objects.equals(keyword, "")){
            cours=coursService.getAllCoursByActiveTrueAndProfesseur (professeurRepository.findByUsername(data),PageRequest.of(page,size));
        }
        assert cours != null;
        Page<CourResponseDto> dataDto = cours.map(CourResponseDto::toDto);
